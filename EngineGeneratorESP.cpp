...
auto EngineGeneratorESP::GenerateESPEngine(const string& path)->shared_ptr<Optimizer>
{
	...
	shared_ptr<ISelection> instSelection = make_shared<UniqueParentsSelection>(instMtInt, constsES.NumberOfRecombParents);
	...
	shared_ptr<Optimizer> engineESP = make_shared<OptimizerESP>(instTable, constsES,
		constsESP, instMtDouble, instSelection, instRecomb, instMutation);
