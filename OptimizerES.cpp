...
auto OptimizerES::GenRndParams(shared_ptr<IRandDouble> rnd) const->
decltype(declval<ParamsIndividual>().CopyParams())
{
...
auto OptimizerES::WriteFitnessEntryToFile(ofstream& fileObject, const double& fitness, const vector<double>& mappedPrams,
	const std::vector<double>& sigmas) const->void
{
	fileObject << std::scientific << std::setprecision(5) << fitness << "; ";

	for (auto const& e : mappedPrams)
	{
