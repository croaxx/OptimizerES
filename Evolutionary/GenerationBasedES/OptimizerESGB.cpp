...
namespace Evolutionary
{
	namespace GenerationBased
	{
		...
		OptimizerESGB::OptimizerESGB(shared_ptr<ISortedFitnessTable> table,
									 ...
									 shared_ptr<IRandDouble> rnd,
									 shared_ptr<ISelection> selection,
									 shared_ptr<IRecombination> recombination, 
									 shared_ptr<IMutation> mutation):
			OptimizerES{ table, constantsES },_constantsESGB{ constantsESGB }, _rnd{ rnd },
		...
		auto OptimizerESGB::StartOptimization(const shared_ptr<IFitness> fitObject) -> shared_ptr<ISortedFitnessTable>
		{
			...
			while (fitCounter < _constantsES.MaxFitCalls)
			{
				...
				for (unsigned j = 0; j < _constantsESGB.ChildrenNumber; j++)
				{
					...
					OptimizerES::WriteFitnessEntryToFile(fileObject, fitnesses.back(), 
														 mappedParams, recombined.GetSigmasRef());
					++fitCounter;
