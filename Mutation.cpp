...
StandardMutation::StandardMutation(shared_ptr<IRandDouble> rand, 
	const double tau0, const double tau1):	_rand{ rand }, _tau0{ tau0 }, _tau1{ tau1 } {};

auto StandardMutation::Mutate(ParamsIndividual& indiv) -> void
{
	// Mutate Sigma
	auto rnd = _rand->NextRand();
	auto commonFactor = exp(_tau0*rnd);
	auto sigmas = indiv.CopySigmas();
	auto params = indiv.CopyParams();
	for_each(begin(sigmas), end(sigmas), [&](decltype(sigmas[0]) s)
	{
		rnd = _rand->NextRand();
		auto sepFactor = exp(_tau1*rnd);
